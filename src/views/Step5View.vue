...
const animate = (item, index) => {
  const delay = Math.random() * 0.3 + 0.2

  const patterns = item.classList.contains('parallax__item__title')
    ? [
        { x: 100, y: 0 },
        { x: -100, y: 0 },
      ]
  ...
  gsap.fromTo(
    item,
    { opacity: 0, x: pattern.x, y: pattern.y },
    {
      opacity: 1,
      x: 0,
      y: 0,
      duration: 1.25,
      // animation이 여러번 실행되었을 때 자동으로 한번만 실행되게 해줌
      overwrite: 'auto',
      ease: 'expo',
      delay: delay,
    },
...
<template>
  <main id="parallax__cont">
    <section v-for="(section, index) in sections" :key="index" class="parallax__item">
      ...
      <h2 class="parallax__item__title reveal">section{{ index + 1 }}</h2>
      ...
      <p class="parallax__item__desc reveal">{{ section.desc }}</p>
    </section>
  </main>
</template>
